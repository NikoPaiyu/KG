...
@Component({
  selector: "app-process-answer-status-list",
  templateUrl: "./process-answer-status-list.component.html",
  styleUrls: ["./process-answer-status-list.component.scss"],
})
export class ProcessAnswerStatusListComponent implements OnInit {
  questionId = null;
  showPreview = false;
  previewData = "";
  list = {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe((params) => {
      ...
      if (questionId) {
        this.question
          .getAnswerStatusListById(questionId)
          .subscribe((response) => {
  ...
  getNoteslist() {
    this.question
      .getAnswerStatusListNotes(this.questionId)
      .subscribe((res: any) => {
        this.notesList = res;
  ...
  _getADDnotePermission() {
    if (this.qDetail.status === "APPROVED") {
      return false;
    }
    return true;
  }
