...
export class BillManagementService {
  ...
  publishErrata(erratumId) {
    const url = this.apiBaseURI + `/errata/publish/${erratumId}`;
  ...
  getBillByBillIdwithAmend(billId) {
    const url = this.apiBaseURI + `/getById/${billId}`;
    return this.http.get(url);
  }
  getBillById(billId) {
  ...
  getAllBillsNotAddedInPriorityList(assemblyId, sessionId, categoryId) {
    const url = this.apiBaseURI + `/priorityList/list/unAssigned/bills/category?assemblyId=${assemblyId}&sessionId=${sessionId}&catgoryId=${categoryId}`;
    return this.http.get(url);
  }
  ...
  getApprovedBillListByBillId(billId) {
    const url = this.apiBaseURI + `/act/getReferencedActs?billId=${billId}`;
    return this.http.get(url);
  }
  ...
  publishTranslatedBills(id) {
    const url = this.apiBaseURI + `/translation/publish/${id}`;
  ...
  getGoverRecommendedBills() {
    const url = this.apiBaseURI + `/all/governorRecommendationList`;
    return this.http.get(url);
  }
  getBillsForLateDocSubmission() {
    const url = this.apiBaseURI + `/all/uploadedStatement`;
    return this.http.get(url);
  }
}
