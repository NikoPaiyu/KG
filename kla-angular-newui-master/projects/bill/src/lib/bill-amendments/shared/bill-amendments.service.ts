...
export class BillAmendmentsService {
 ...
 // obj-intro services
  submitClauseByClauseAmendments(amendment) {
    const url = this.apiBaseURI + `/clause`;
    return this.http.post(url, amendment);
  }
  getAmendmentsOfBillByMember(billId, memberId) {
    const url = this.apiBaseURI + `/clause/getByMemberId?memberId=${memberId}&billId=${billId}`;
    return this.http.get(url);
  }

submitAmendments(body) {
  const url = this.apiBaseURI + `/clause/submitToSO`;
  return this.http.post(url, body);
}
updateAmendmentStatus(billId, body) {
  const url = this.apiBaseURI + `/clause/amendment/section/update?billId=${billId}`;
  return this.http.put(url, body);
}
deleteAmendments(amendmentId) {
  const url = this.apiBaseURI + `/clause/deleteByClauseId?clauseId=${amendmentId}`;
  return this.http.delete(url);
}
getClauseBymemberId() {
    const url = this.apiBaseURI + `/clause/getByMemberId`;
    return this.http.get(url);
}
getClauseByuserIdandStatus(body= {}, isAssistant) {
  if (isAssistant) {
    const url = this.apiBaseURI + `/clause/getAllBillByFilterForAssistant`;
    return this.http.post(url, body);
  } else {
...
getClauseNoticesListByBillId(body) {
  const url = this.apiBaseURI + `/clause/getByBillId`;
  return this.http.post(url, body);
  }
  ...
  assignToAssistant(body) {
    const url = this.apiBaseURI + `/clause/assignToAssistant`;
    return this.http.post(url, body);
  }
  intitialList(body, billId, listType) {
    const url = this.apiBaseURI + `/clause/intialList?billId=${billId}&type=${listType}`;
    return this.http.post(url, body);
  }
  createClauseList(body) {
    const url = this.apiBaseURI + `/clause/list`;
    return this.http.put(url, body);
  ...
  publishList(listId) {
    const body = {};
    const url = this.apiBaseURI + `/clause/list/publish?listId=${listId}`;
    return this.http.put(url, body);
  }
  resubmitFile(body) {
  ...
  applyAmendment(clauseId) {
    const url = this.apiBaseURI + `/clause/applyClauseAfterSabha?clauseId=${clauseId}`;
    return this.http.get(url);
  }
