...
export class ClauseByClauseNoticesForListComponent implements OnInit {
  ...
  resubmitFile(activeSubType) {
    ...
    this.billAmendmentsService.resubmitFile(body).subscribe( res => {
        if (res) {
          this.router.navigate(['business-dashboard/bill/file-view/', this.totalClauseResponse.billFileId]);
  ...
  this.billAmendmentsService.publishList(this.listData.listId).subscribe( res => {
      if (res) {
        ...
        this.notification.create('Success', 'List Published successfully!', '');
      }
    });
  this.visibleModel = false;
  this.getclauseNoticesList(this.billId);
