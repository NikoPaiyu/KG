...
export class ClauseByClauseNoticesComponent implements OnInit {
  ...
  markAcceptReject(accept, noticeId) {
    ...
    if (accept) {
      this.billAmendmentsService.markAmendmentAsAccepted(body).subscribe(data => {
        this.getclauseNoticesList(this.billId);
        this.notification.success('Success', 'Amendment(s) Accepted successfully');
      });
    } else {
      this.billAmendmentsService.markAmendmentAsRejected(body).subscribe(data => {
        this.getclauseNoticesList(this.billId);
