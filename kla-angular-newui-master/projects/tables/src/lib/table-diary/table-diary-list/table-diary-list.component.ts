...
@Component({
  selector: 'tables-table-diary-list',
  templateUrl: './table-diary-list.component.html',
  styleUrls: ['./table-diary-list.component.css']
})
export class TableDiaryListComponent implements OnInit {
  ...
  getTableDiaryList(){
    if ( this.assemblyId && this.sessionId ) {
      this.TablediaryService.getTableDiaryList(
          this.assemblyId,
          this.sessionId
        )
        .subscribe((Res: any) => {
  ...
  gotoView(id){
   this.router.navigate(['business-dashboard/tables/prepare-table-diary',id])
  }
}
