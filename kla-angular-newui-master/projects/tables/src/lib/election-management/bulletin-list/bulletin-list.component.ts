...
export class BulletinListComponent implements OnInit {
  ...
  setRBSPermisson() {
    if (this.common.doIHaveAnAccess('PUBLISH_BULLETIN', 'READ')) {
      this.permissions.viewPublishedBulletins = true;
    }
    if (this.common.doIHaveAnAccess('FILE', 'READ')) {
      this.permissions.viewFile = true;
    }
    if (this.common.doIHaveAnAccess('PUBLISH_BULLETIN', 'CREATE')) {
      this.permissions.publishBulletin = true;
    }
  }
  getBulletinList() {
    if (this.permissions.viewPublishedBulletins) {
  ...
  getAllBulletinList() {
    this.electionService.getAllBulletinList().subscribe((Response: any) => {
      if (Response) {
  ...
  getApprovedBulletinList() {
    this.electionService.getPublishedBulletinList().subscribe((Response: any) => {
      if (Response) {
        this.listOfData = this.listOfAllData = Response;
  ...
  openBulletin(id) {
    this.electionService.getBulletinById(id).subscribe(res => {
  ...
  publishBulletin() {
    this.electionService.publishBulletin(this.bulletinData.id).subscribe((res => {
      ...
      this.openBulletin(this.bulletinData.id);
      this.getBulletinList();
    }));
  }
}
