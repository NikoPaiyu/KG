...
@Component({
  selector: 'lib-resolution-amendment-notices',
  templateUrl: './clause-by-clause-notices.component.html',
  styleUrls: ['./clause-by-clause-notices.component.css']
})
export class ResoolutionAmendmentNoticesComponent implements OnInit {
  ...
  markAcceptRejectFlag: boolean;
  secondReadingFlag;
  convertToOfficialAmendmentFlag: boolean;
  constructor(
  ...
  private pmbillmanagement: BillManagementService,
  ...
  getclauseNoticesList(id) {
    ...
    if (this.totalClauseResponse.list3status !== 'PUBLISHED') {
      this.convertToOfficialAmendmentFlag = true;
    } else { this.convertToOfficialAmendmentFlag = false; }
    ...
    this.secondReadingFlag = differenceInCalendarDays(parseISO(this.totalClauseResponse.schedule.secondReading), this.today);
  ...
  showLinks(list) {
    if (this.secondReadingFlag < 0 && list.status !== 'ACCEPTED' && list.status !== 'REJECTED'
        && this.totalClauseResponse.billStatus === 'PASSED' && this.totalClauseResponse.list3status === 'PUBLISHED') {
      this.markAcceptRejectFlag = true;
    } else { this.markAcceptRejectFlag = false; }
    if (this.tableParams.colSpan) {
      return;
    }
    this.clauseNoticesList.forEach((element) => {
  ...
  getAssistantList() {
    this.commonService.getAssisstantList(['PMBR_ASSISSTANT']).subscribe((Res: any) => {
      if (Res) {
  ...
  onSubmitAssistant() {
    ...
    this.billAmendmentsService.assignToAssistant(body).subscribe((res: any) => {
      if (res) {
        ...
        this.router.navigate(['business-dashboard/pmbr/assigned-resolution-amendments'])
      }
    });
  ...
  //     billId : this.billId
  //   };
  //   this.billAmendmentsService.intitialList(body, this.billId).subscribe( res => {
  //       if (res) {
  ...
  resubmitFile(activeSubType) {
    ...
    this.billAmendmentsService.resubmitFile(body).subscribe( res => {
        if (res) {
          this.notify.success(
            'Success', 'Resubmitted successfully.'
          );
          this.router.navigate(['business-dashboard/bill/file-view/', this.totalClauseResponse.fileId]);
        }
      });
    } else {
  ...
  this.billAmendmentsService.publishList(this.listData.listId).subscribe( res => {
      if (res) {
        ...
        this.notify.success(
          'Success', 'List published successfully.'
        );
      }
    });
  this.visibleModel = false;
  ...
  markAcceptReject(accept, noticeId) {
    const noticeIds = [];
    const checkedArray = this.clauseNoticesList.filter(
      (item) => this.mapOfCheckedId[item.id]
    );
    // const noticeIds = checkedArray.map(x => x.id);
    noticeIds.push(noticeId);
    const body = {
      noticeIds
    };
    if (accept) {
      this.pmbillmanagement.markAmendmentAsAccepted(body).subscribe(data => {
        this.getclauseNoticesList(this.billId);
        this.notification.success('Success', 'Amendment(s) Accepted successfully');
      });
    } else {
      this.pmbillmanagement.markAmendmentAsRejected(body).subscribe(data => {
        this.getclauseNoticesList(this.billId);
        this.notification.success('Success', 'Amendment(s) Rejected successfully');
      });
    }
  }
  convertToOfficialAmendment() {
    const checkedArray = this.clauseNoticesList.filter(
      (item) => this.mapOfCheckedId[item.id]
    );
    const noticeIds = checkedArray.map(x => x.id);
    ...
    this.billAmendmentsService.convertToOfficialAmendment(body).subscribe(data => {
      this.notification.success('Success', 'converted to official Amendment(s)');
    });
  }
  goBack() {
    window.history.back();
