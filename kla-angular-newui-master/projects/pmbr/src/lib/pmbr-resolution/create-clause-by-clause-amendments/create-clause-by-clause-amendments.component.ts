...
import { BillBlockService } from '../shared/services/bill-block.service';
...
export class ResolutionCreateClauseByClauseAmendmentsComponent implements OnInit {
  isView = false;
  ...
  billStatus;
  canAttachToFile: any;
  tags = { title: "Paragraph", code: "TEXT", id: 30, color: "purple" };
  ...
  getRbsPermissionsinList() {
    ...
    if (this.commonService.doIHaveAnAccess('RESOLUTION_AMENDMENT', 'SUBMIT')) {
    ...
    this.amendmentService
      ...
      .subscribe((data) => {
        if (data) {
          ...
          if (response) {
            this.resolutionDetails = response;
  ...
  getAmendmentsOfBill(billId, memberId) {
    if (this.allowOralAmendment) {
      ...
      this.amendmentService
        ...
        .subscribe((res: any) => {
          this.amendments = res;
          let status = this.amendments.filter( x => x.status === 'SAVED');
          if(status.length > 0) {
  ...
  onSelectText(block) {
    if (this.showPreview || this.disableActions || block.status === 'INACTIVE') {
      return;
    }
  ...
  insertblockAmendment(index, typeId, blockId, parentBlock, currentBlock, type) {
  ...
  applyAmendment(amendment) {
    if (amendment.amendmentType === 'AMENDMENT') {
      this.amendmentService.applyAmendment(amendment.id, this.currentUser.userId).subscribe(async (data) => {
        this.getBill(this.billId);
  ...
  isEditable(item) {
    ...
    if (item.status === 'SAVED' && (this.isMLA() || this.isPPO() || this.isAssistant()) && editableItems.indexOf(item.operationType) > -1) {
      editable = true;
    }
    if (item.status == 'ASSIGNED' && this.isAssistant() && editableItems.indexOf(item.operationType) > -1) {
      editable = true;
    }
    return editable;
  }
  ...
  resubmitFile() {
    ...
    this.file.attachToFile(body).subscribe(data => {
      if (data) {
        this.notify.success('Success', 'Attached to file successfully');
        this.router.navigate(['business-dashboard/pmbr/file-view/', this.billFileId]);
      }
    });
  }
