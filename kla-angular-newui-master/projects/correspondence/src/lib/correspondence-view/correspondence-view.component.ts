...
@Component({
  selector: 'correspondence-correspondence-view',
 ...
 @ViewChild(DynamicFormComponent, { static: false })
  ...
  getCorrespondenceById() {
    this.correspondenceService
      ...
      .subscribe((Res) => {
        ...
        if(this.correspondenceData.businessCode=="COMMITTEE_NOMINEE_SUBMISSION"){
          this.validateForm.addControl('businessData',this.fb.array([]));
        }
        if (this.purpose) {
  ...
  updateCorrespondence() {
    ...
    if (this.form) {
      ...
      if(this.correspondenceData.businessCode == 'TABLE_OBITURY_FAMILY_LETTER'){
      console.log(this.validateForm.value.toValue);
      this.toValue =[{
        id: this.correspondenceData.toCode[0].id,
        code:"",
        displayName:this.validateForm.value.toValue,
      }]
      }
      if (this.toData.length <= 0) {
        ...
        for (let i = 0; i < this.toValue.length; i++) {
          if(this.correspondenceData.businessCode == 'TABLE_OBITURY_FAMILY_LETTER'){
            this.toData.push( this.toValue[i]);
          }else{
          this.typeList.forEach((el) => {
      ...
      this.correspondenceService.saveCorrespondence(this.validateForm.value).subscribe( Response => {
        if (Response) {
          ...
          if (this.correspondenceData.status === 'APPROVAL_PENDING') {
            ...
            this.isEdit = false;
            this.router.navigate(['/business-dashboard/correspondence/correspondence-list']);
          }
        }
      });
    }
      ...
      for (const key in this.validateForm.controls) {
        ...
        if (key === "businessData") {
          const control = this.validateForm.get("businessData") as FormArray;
          // tslint:disable-next-line: forin
    ...
    this.correspondenceService
      ...
      .subscribe((Response: any) => {
        if (Response) {
          ...
          this.router.navigate([
            'business-dashboard/correspondence/correspondence-list'
          ]);
        }
      });
  }
  ...
  loadPermissions() {
    if (
      this.correspondenceService.doIHaveCorrespondenceAccess('CORRESPONDENCE', 'UPDATE')
    ) {
      this.editButton = true;
    }
    if (
      this.correspondenceService.doIHaveCorrespondenceAccess('CORRESPONDENCE', 'SUBMIT')
    ) {
  ...
  showReference(id) {
    ...
    // }
    if (this.correspondenceData.businessReferType === 'BILL') {
     this.router.navigate([
       'business-dashboard/bill/bill-view',
       id
      ]);
    } else if (this.correspondenceData.businessReferType === 'PRIORITY_LIST') {
      this.router.navigate([
        'business-dashboard/bill/view-priority-list',
        id
      ]);
    } else if (this.correspondenceData.businessReferType === 'PM_RESOLUTION_LETTER_TO_STAKE_HOLDER') {
      this.router.navigate(['business-dashboard/pmbr/amendments-list-view', id]);
    } else if (this.correspondenceData.businessReferType === 'PM_BILL') {
      this.router.navigate(['business-dashboard/pmbr/resolution-view',id]);
    } else if (this.correspondenceData.businessReferType === 'PMBR') {
...
download() {
  ...
  viewFile() {
    if(this.user.correspondenceCode.type == 'PPO'){
      return;
    }
    if (this.correspondenceData.businessReferType === 'COMMITTEE') {
      this.router.navigate(['/business-dashboard/committee/file-view/',  this.correspondenceData.fileId]);
    } else if(this.correspondenceData.businessReferType === 'TABLE'){
      this.router.navigate(['/business-dashboard/tables/file-view/',  this.correspondenceData.fileId]);
    } else if (this.correspondenceData.businessReferType === 'BILL') {
      this.router.navigate(['business-dashboard/bill/file-view/',  this.correspondenceData.fileId]);
    } else if (this.correspondenceData.businessReferType === 'BUDGET') {
      this.router.navigate(['business-dashboard/budgets/file-view/',  this.correspondenceData.fileId]);
    } else if (this.correspondenceData.businessReferType === 'PMBR') {
      this.router.navigate(['/business-dashboard/pmbr/file-view/',  this.correspondenceData.fileId]);
    } 
    else if (this.correspondenceData.businessReferType === 'PM_BILL') {
      this.router.navigate(['/business-dashboard/pmbr/file-view/',  this.correspondenceData.fileId]);
    } 
    else {
      this.router.navigate(['business-dashboard/cpl/file-workflow/',  this.correspondenceData.fileId]);
    }
  }
  setBusiness(){
    if(this.correspondenceData.businessCode=="COMMITEE_REQUEST_NOMINEES"){
      let toPpoData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(toPpoData && !toPpoData.isReplaied && !toPpoData.actionRequired){
        return 'COMMITTEE_NOMINEE_SUBMISSION';
      }
      else{
        return 'NO_BUSINESS';
      }
    }
    else if(this.correspondenceData.businessCode=="COMMITTEE_MEETING_SUPPORTING_DOCUMENT_REQUEST"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'COMMITTEE_MEETING_SUPPORTING_DOCUMENT_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="TABLE_CORRESPONDANCE_LAW_DEPT"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'TABLE_GOVERNORS_SPEECH';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="BUDGET_AP_BILL_REQUEST"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'BUDGET_AP_BILL_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="VOA_AP_BILL_REQUEST"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'VOA_AP_BILL_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="SDG_AP_BILL_REQUEST"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'SDG_AP_BILL_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="BUDGET_DOCUMENT_LETTER"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'BUDGET_DOCUMENT_REPLY_LETTER';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="BUDGET_DOCUMENT_GRL_REQUEST_LETTER"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'BUDGET_DOCUMENT_GRL_REPLY_LETTER';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="BUDGET_SDFG_LETTER"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'BUDGET_SDFG_LETTER_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="BUDGET_SDG_EG_REQUEST_LETTER"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'BUDGET_SDG_EG_REPLY_LETTER';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="SDG_EG_TIME_ALLOCATION_LETTER"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'SDG_EG_TIME_ALLOCATION_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
    }else if(this.correspondenceData.businessCode=="TABLE_BUSINESS_TIME_ALLOCATION"){
      let deptData = this.correspondenceData.toCode.find(element => element.toCode == this.code.toString());
      if(deptData && !deptData.isReplaied && !deptData.actionRequired){
        return 'TABLE_BUSINESS_TIME_ALLOCATION_RESPONSE';
      }
      else{
        return 'NO_BUSINESS';
      }
  ...
  viewMaster() {
    this.router.navigate([
      'business-dashboard/correspondence/correspondence', 'view',
      this.correspondenceData.masterLetter,
    ]);
    this.correspondenceId = this.correspondenceData.masterLetter;
    this.getCorrespondenceById();
  }
