...
@Component({
  selector: 'Correspondence-table-timeallocation-response',
  templateUrl: './table-timeallocation-response.component.html',
  styleUrls: ['./table-timeallocation-response.component.css']
})
export class TableTimeallocationResponseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.correspondenceFormGrp.value.businessReferId){
      this.businessId = this.correspondenceFormGrp.value.businessReferId;
    }
    this.getDayAndDates();
  ...
  getDayAndDates() {
    this.correspondence.getBusinessDayAndDates(this.businessId).subscribe((res: any) => {
      if (res) {
  ...
  getTimeByUserId() {
    this.correspondence.getBusinessTimeByUserId(this.businessId).subscribe((timeData: any) => {
      if (timeData) {
  ...
  getByFileIdAndDay(dateObj) {
    if (dateObj) {
      this.correspondence.getByBuisnessIdAndDay(this.businessId,dateObj.key).subscribe((res: any) => {
  ...
  patchFormValues(selDay, timeConf) {debugger;
